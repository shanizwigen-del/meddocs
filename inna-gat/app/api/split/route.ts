...
async function processGroups(buffer: Buffer, originalFilename: string, groups: number[][], tempUrl: string, memberId: string | null) {
  for (let i = 0; i < groups.length; i++) {
    try {
      ...
      const rows = await sql`
        INSERT INTO documents (blob_url, filename, status, member_id)
        VALUES (${blob.url}, ${`עמודים ${groups[i].join(',')} — ${originalFilename}`}, 'processing', ${memberId})
        RETURNING id
      `
      const id = rows[0].id
...
export async function POST(req: NextRequest) {
  ...
  const { blobUrl, filename, memberId } = await req.json()
  ...
  if (numPages === 1) {
    const rows = await sql`INSERT INTO documents (blob_url, filename, status, member_id) VALUES (${blobUrl}, ${originalFilename}, 'processing', ${member}) RETURNING id`
    waitUntil((async () => {
  ...
  waitUntil(processGroups(buffer, originalFilename, groups, blobUrl, member))
