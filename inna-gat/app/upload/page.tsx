...
export default function UploadPage() {
  ...
  // קריאת בן המשפחה מכתובת ה-URL (?member=...) וטעינת רשימת בני המשפחה
  useEffect(() => {
    const fromUrl = new URLSearchParams(window.location.search).get('member')
  ...
  const backHref = memberId ? `/member/${memberId}` : '/'
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center" dir="rtl">
      <div className="max-w-lg w-full px-4 space-y-4">
        ...
        {memberName ? (
          <div className="bg-blue-50 border border-blue-200 rounded-xl px-4 py-3 text-center text-sm text-blue-700">
            מעלה לתיק של <span className="font-semibold">{memberName}</span>
          </div>
        ) : (
          <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 space-y-2">
            <p className="text-sm text-amber-800">בחרי לאיזה בן משפחה לשייך את המסמכים:</p>
            <select
              value={memberId ?? ''}
              onChange={e => setMemberId(e.target.value || null)}
              className="w-full border rounded-lg px-3 py-2 text-sm bg-white"
            >
              <option value="">— בחרי בן משפחה —</option>
        ...
        <button onClick={() => router.push(backHref)} className="w-full text-gray-500 text-sm">
          → חזרה
        </button>
      </div>
    </div>
